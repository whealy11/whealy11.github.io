...
## Final Intuitions
If you don't remember all the math, the intuition is the following. We've made a non-linear function a linear one in a higher dimensional space. By doing this, we've replaced a latency bottleneck in attention with a simple running sum of outer products, enabling us to compute $f(x_n)$ very easily from $f(x_{n-1})$. Since the softmax operation used in attention requires infinite dimensionality to linearize, our vectors must be infinite. We use tricks to approximate these infinite dimensional linear operations in finite dimensions.
